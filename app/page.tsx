...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-[#0f1015] text-white flex flex-col items-center relative overflow-x-hidden font-sora select-none">
      ...
      {/* MENU - COM FIBRA ÓTICA E CONTORNO */}
      <header className="fixed top-6 z-[100] w-full max-w-5xl px-4">
        <nav className="relative bg-zinc-950/90 border border-zinc-800 rounded-full px-8 py-3 flex items-center justify-between shadow-2xl overflow-hidden group">
          <div className="absolute inset-0 rounded-full varko-beam-overlay animation-beam-azul opacity-50 group-hover:opacity-100 transition-opacity"></div>
      ...
      <section id="home" className="min-h-screen flex flex-col items-center justify-center text-center relative w-full max-w-5xl px-6 pt-20">
        ...
        <motion.div whileHover={{ scale: 1.05 }} className="relative z-10 rounded-full p-[1px] overflow-hidden group">
          <div className="absolute inset-0 rounded-full varko-beam-overlay animation-beam-azul opacity-60 group-hover:opacity-100 transition-opacity"></div>
          <Link href="/login" className="relative flex items-center gap-3 bg-transparent hover:bg-[#12f2f2] text-white hover:text-black px-12 py-5 rounded-full font-bold text-xl border border-white/10 transition-all">
            Iniciar Análise SWOT <ArrowRight className="w-6 h-6" />
          </Link>
        </motion.div>
      </section>

      {/* 2. SEÇÃO SOBRE - REVEAL CSS PURO (INFALÍVEL) */}
      <section id="sobre" className="w-full flex flex-col items-center py-20 border-t border-zinc-900/50">
        <div className="py-20 px-6 reveal-block">
          <p className="max-w-4xl text-center text-xl md:text-[28px] font-light leading-relaxed text-zinc-400">
        ...
        <div className="py-20 px-6 reveal-block">
          <p className="max-w-4xl text-center text-[20px] md:text-[24px] font-light leading-relaxed text-zinc-500">
        ...
        <div className={`grid gap-10 w-full transition-all duration-700 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-3' : 'grid-cols-1 max-w-[500px]'}`}>
          {servicos.map((s) => (
            <div key={s.id} className="relative group rounded-[32px] overflow-hidden aspect-video shadow-2xl">
              <div className="absolute inset-0 rounded-[32px] varko-beam-overlay animation-beam-azul opacity-30 group-hover:opacity-90 transition-opacity z-20"></div>

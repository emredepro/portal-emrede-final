    ...
    <main className="bg-[#0f1015] text-white flex flex-col items-center relative font-sora select-none overflow-x-hidden">
      ...
      <section id="home" className="min-h-screen flex flex-col items-center justify-center text-center relative w-full max-w-5xl px-6">
        ...
        <div className="relative z-10 rounded-full p-[1px] overflow-hidden group shadow-2xl transition-transform hover:scale-105">
          ...
          <Link href="/login" className="relative flex items-center gap-3 bg-[#0f1015] hover:bg-[#12f2f2] text-white hover:text-black px-12 py-5 rounded-full font-bold text-xl transition-all">
            Iniciar Análise SWOT <ArrowRight className="w-6 h-6" />
          </Link>
        </div>
      </section>

      {/* 2. SEÇÃO SOBRE - SCROLL TRIGGER REAL (PIN + REVEAL) */}
      <ScrollTriggerSobre />
...
function ScrollTriggerSobre() {
  ...
  // Efeito de suavização para o scroll não ficar "picotado"
  const smoothProgress = useSpring(scrollYProgress, { stiffness: 100, damping: 30, restDelta: 0.001 });

  // Mapeamento do Scroll Trigger:
  // Texto 1: Aparece (0-0.2), Fica (0.2-0.4), Some (0.4-0.5)
  const opacity1 = useTransform(smoothProgress, [0, 0.2, 0.4, 0.5], [0, 1, 1, 0]);
  const scale1 = useTransform(smoothProgress, [0, 0.2], [0.9, 1]);

  // Texto 2: Aparece (0.55-0.75), Fica (0.75-0.9), Some (0.95-1)
  const opacity2 = useTransform(smoothProgress, [0.5, 0.65, 0.85, 1], [0, 1, 1, 0]);
  const scale2 = useTransform(smoothProgress, [0.5, 0.65], [0.9, 1]);

  return (
    <section ref={targetRef} className="relative h-[300vh] w-full bg-[#0f1015]">
      <div className="sticky top-0 h-screen w-full flex flex-col items-center justify-center px-6 overflow-hidden">
        
        {/* TEXTO 1 - 28px */}
        <motion.div style={{ opacity: opacity1, scale: scale1 }} className="max-w-4xl absolute text-center">
        ...
        {/* TEXTO 2 - 24px */}
        <motion.div style={{ opacity: opacity2, scale: scale2 }} className="max-w-4xl absolute text-center">
